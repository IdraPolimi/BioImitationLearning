...
%PICTURE=ALPhotoCaptureProxy(IP,PORT);
...
POS_EFF_REWARD=[0.1000;0.0100;0.1800; 0.4929;-0.3573;-1.5391]; %TESTA NAO
%POS_EFF_REWARD=[10;20;10;0.54;0.213;0.1231];
% POS_EFF=POS_EFF(1:3);
% if(index<=9)
...
x=cell2mat(MOTION.getAngles(NAMES, false));
x=[x;POS_EFF_REWARD];
disp('stato')
size(x)
pi_pesi=randn(NUM_PRIMITIVE,1);
...
while true
    ...
    %randomPosEffReward(1) =  -0.59266+ rand(1,1)*(-0.19266+0.59266); %eRoll: -0.59266, -0.19266
    %randomPosEffReward(2) =  -0.70473+ rand(1,1)*(-0.30473+0.70473);     %LElbowYaw -0.70473,-0.30473
    %randomPosEffReward(3) =  0.09295+ rand(1,1)*(0.49295-0.09295);   %LShoulderPitch  0.09295,0.49295
    %randomPosEffReward(4) =  0.64826+ rand(1,1)*(1.04826-0.64826);%LShoulderRoll'; 0.64826,1.04826
    %randomPosEffReward
  %  POS_EFF_REWARD = fLeftHand([randomPosEffReward(3);randomPosEffReward(4);randomPosEffReward(2);randomPosEffReward(1)]);
    %   Per falo vedere sulla console
...
%     figure(1003)
%     clf
    pi_f=pi_base((1:DOF),delta_t, goalRel(1:DOF));
%     for i=1:NUM_PRIMITIVE-2
%         if(pi_pesi(i)>soglia)
%             %   Ogni primitiva ha 4 DOF con un ID [i i+1 i+2 i+3]
%             prim=pi_base((1:DOF)+4*(i-1),delta_t);
%             %coupling term per sistema canonico
% %             for j=1:DOF
% %             vecmax(j)=prim(j,i)-pos_corrente(j);
% %             end
% %             delta_t=delta_t*(1/(1+10*(min(min(vecmax)))));
% 
...
% %             hold on
% %         plot(pi_pesi(i)*prim(1,:)/sum(pi_pesi(1:NUM_PRIMITIVE)),'-');
% %          text(400,pi_pesi(i)*prim(1,400)/sum(pi_pesi(1:NUM_PRIMITIVE)),sprintf('%d',pi_pesi(i)))
%         end
%     end
%     pi_f =pi_f/sum(pi_pesi(1:NUM_PRIMITIVE-2));
% %     figure(1001)
% %     hold on;
% %     plot(pi_f(1,1:500),'*')
% %     
...
%         if(pi_pesi(i)>soglia)
%             primYaw=pi_base((1:DOF)+4*(i-1),delta_t);
%             %coupling term per sistema canonico
% %             for j=1:DOF
% %             vecmax(j)=primYaw(j,i)-pos_corrente(j);
% %             end
% %             delta_t=delta_t*(1/(1+10*(min(min(vecmax)))));
...
%     end
   ...
   % r=Calcola_reward();
    ...
    %r1=randi(100,1);
    MOTION.setPosition('LArm',int8(0),pos_iniziali(:,mod(check+ 1,4)+1)',0.9,int8(63))
    pause(5)
    %MOTION.angleInterpolationWithSpeed(NAMES, stato(r1,:),0.5);
   % pause(3)
%     if(index<=9)
%         img = double(im_f(index).sig/255);
%         %img = double(cameraNao().sig/255);
...
%     y=output;
    ...
    y=cell2mat(MOTION.getAngles(NAMES, false));
    y=[y;POS_EFF_REWARD];
%     i=i+1;
    %-------------------------------------------------------------------------
